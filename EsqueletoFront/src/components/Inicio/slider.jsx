...
export const Slider = ({ slides }) => {
  ...
  useEffect(() => {
    ...
    axios.get("localhost:1337/api/blogs?populate=*")
      .then((response) => {
          ...
          {posts.map((post, index) => (
            <SwiperSlide key={index}>
              <div className='container_noticias'>
                ...
                {`http://localhost:1337${post?.attributes.image.data.attributes.formats.large.url}`} alt={post?.attributes?.titulo} loading="lazy" /             
                   </div>
